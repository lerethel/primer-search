...
export async function initSearch(req, res) {
  ...
  if (!retryURL) {
    ...
    return senderBySelector[".error, .info"](
      req,
      res,
...
export async function getPrimers(req, res) {
  ...
  for (const selector in senderBySelector) {
    const element = document.querySelector(selector);
    if (element) {
      return senderBySelector[selector](req, res, element);
    }
  }
}
